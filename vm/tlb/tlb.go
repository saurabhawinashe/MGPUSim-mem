...
import (
	...
	"gitlab.com/akita/mem/v2/vm/tlb/internal"
...
func (tlb *TLB) handleTranslationHit(
	...
	tracing.AddTaskStep(tracing.MsgIDAtReceiver(req, tlb), now, tlb, "hit")
	tracing.TraceReqComplete(req, now, tlb)
...
func (tlb *TLB) handleTranslationMiss(
	...
	if fetched {
		tlb.topPort.Retrieve(now)
		tracing.TraceReqReceive(req, now, tlb)
		tracing.AddTaskStep(tracing.MsgIDAtReceiver(req, tlb), now, tlb, "miss")
		return true
	}
...
func (tlb *TLB) processTLBMSHRHit(
	...
	tlb.topPort.Retrieve(now)
	tracing.TraceReqReceive(req, now, tlb)
	tracing.AddTaskStep(tracing.MsgIDAtReceiver(req, tlb), now, tlb, "mshr-hit")
